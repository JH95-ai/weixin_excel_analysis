...
DF=DataFrame(read_excel,columns=['人气','小程序介绍'])
...
stopwords=pd.read_txt('E:\\pycharm_object\\weixin_excel_analysis\\ChineseStopWords-master\\ChineseStopWords.txt',index_col=False,quoting=3,sep='\t',names=['stopword'],encoding='ISO-8859-1')
print(stopwords)
...
plt.bar(word[:20], count[:20], color='lightskyblue')#显示前十分类
plt.xlabel('出现词语', FontProperties=font)
plt.ylabel('出现次数', FontProperties=font)
plt.title('人气前一万小程序介绍词频分析', FontProperties=font)
plt.show()